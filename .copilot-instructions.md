...
All documentation for the Attendr project is stored in the **/docs** folder at the root of the repository. This includes:

**Technical Documentation** (`/docs/technical/`):
- Domain model documentation (e.g., StatefulDomainModel)
...
**Component-Specific Documentation** (`/docs/components/`):
- Individual service/module documentation (conferences, groups, profiles, etc.)
...
- **Front-end Architecture**: 
  ...
  - Authorization: Auth0 using the angular-auth-oidc-client library
...
8. **Front-end Development**:
  ...
  - Use Auth0 with the angular-auth-oidc-client library for authentication/authorization flows

### Project Organization Standards

**CRITICAL: All projects MUST be organized following this standard structure.**

#### Standard Project Structure

Projects should ALWAYS be organized as follows:

1. **Abstractions Project** (`*.Abstractions`)
   - Contains all abstractions for the given module/service
   - Contains all DTOs (Data Transfer Objects)
   - DTOs are preferably implemented as **records**
   - If records are not suitable, use classes
   - Contains all repository interfaces and service contracts
   - No implementation code, only contracts and data structures

2. **Module/Domain Project** (e.g., `HexMaster.Attendr.Conferences`)
   - Contains core domain logic and business rules
   - Organized with the following folder structure:
     - **DomainModels/**: Contains domain entities and aggregate roots
     - **Features/**: Contains feature-specific logic (commands, queries, handlers)
     - **Services/**: Contains domain services (when needed)
   - Implements business rules and domain logic
   - References the Abstractions project

3. **Data Project(s)** (`*.Data.*`)
   - One or more data projects designed to implement repositories
   - Examples: `*.Data.MongoDb`, `*.Data.Postgres`, `*.Data.TableStorage`
   - Contains:
     - **Entities/**: Database-specific entity classes
     - **Mappers/**: Mapping between domain models and entities
     - Repository implementations
   - Each data project is specific to one data store technology
   - References both the Module project and Abstractions project

4. **API Project** (`*.Api`)
   - Contains the ASP.NET Core Web API
   - Always contains an **Endpoints/** folder
   - Endpoints are nicely organized in separate files by feature/domain
   - Uses `MapGroup()` where convenient for organizing related endpoints
   - Example structure:
     ```
     Endpoints/
       ConferenceEndpoints.cs
       PresentationEndpoints.cs
       SpeakerEndpoints.cs
     ```
   - Each endpoint file should use extension methods and MapGroup for clean organization
   - References the Module project and Abstractions project
...
│   ├── Dtos/
│   │   ├── ConferenceDto.cs (record)
│   │   └── PresentationDto.cs (record)
│   └── Repositories/
...
│   ├── Mappers/
│   │   └── ConferenceMapper.cs
│   └── PostgresConferenceRepository.cs
...
- All new projects MUST follow this structure
- When creating new features, place them in the appropriate folder
- Do NOT deviate from this structure without explicit discussion
- Keep concerns separated: domain logic in Module, data access in Data projects, HTTP in API
- DTOs in Abstractions should be simple data containers (prefer records)
- Domain models should contain business logic and validation

### Command and Query Responsibility Patterns

**CRITICAL: This pattern MUST be followed for all backend operations.**

#### Commands (Write Operations)

Commands represent operations that change state in the system (Create, Update, Delete). For ALL command operations:

1. **Always Create/Load a Domain Model**
   - For new entities: Create a new domain model instance using factory methods (e.g., `Conference.Create(...)`)
   - For existing entities: Fetch from the database and reconstruct as a domain model (e.g., `Conference.FromPersisted(...)`)

2. **Apply Changes to Domain Model**
   - Execute business logic and validation through domain model methods
   - Domain models enforce business rules and invariants
   - State changes are tracked by the domain model

3. **Persist to Database**
   - Map the domain model to database entity/document
   - Save to the data store through repository

**Flow**: `Command → Domain Model → Database Entity`

**Example**:
```csharp
// Command handler
public async Task Handle(UpdateConferenceCommand command)
{
    // 1. Load domain model from database
    var conference = await _repository.GetByIdAsync(command.ConferenceId);
    
    // 2. Apply changes via domain model methods
    conference.UpdateDetails(command.Title, command.City, command.Country, ...);
...
Queries represent operations that retrieve data without modifying state. For ALL query operations:

1. **Directly Return DTOs**
   - Query the database and map directly to Data Transfer Objects (DTOs)
   - Do NOT create domain models for read operations
   - Optimize queries for the specific data needed

2. **No Business Logic**
   - DTOs are simple data containers without behavior
   - No validation or business rules in queries

**Flow**: `Query → Database Entity → DTO`

**Example**:
```csharp
// Query handler
public async Task<ConferenceDto> Handle(GetConferenceQuery query)
{
    // Fetch database entity and map directly to DTO
    return await _repository.GetConferenceDtoAsync(query.ConferenceId);
}
```

#### Why This Pattern?

- **Commands**: Domain models enforce business rules and track changes, ensuring data consistency
- **Queries**: Direct DTO mapping is faster and more efficient for read operations
- **Separation**: Clear distinction between operations that change state vs. operations that read state
- **Performance**: Queries avoid unnecessary object creation and business logic overhead
- **Maintainability**: Each operation follows a predictable, consistent pattern

#### Implementation Checklist

When implementing new functionality:

- [ ] Is this a command (write) or query (read) operation?
- [ ] **For Commands**:
  - [ ] Domain model created/loaded?
  - [ ] Business logic applied through domain model methods?
  - [ ] Domain model state tracked correctly?
  - [ ] Persisted through repository?
- [ ] **For Queries**:
  - [ ] Database queried directly?
  - [ ] Mapped to DTO without creating domain model?
  - [ ] No business logic in query?
